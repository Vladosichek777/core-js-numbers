...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(scalarMultiple / (moduleFirst * moduleSecond));
}
...
function getSumToN(n) {
  ...
  return arrNumber.reduce((acc, item) => acc + item);
}
...
function getSumOfDigits(num) {
  ...
  const arrStringNumber = stringNum.split('');
  const arrNumber = arrStringNumber.map((item) => +item);
  const result = arrNumber.reduce((item, acc) => acc + item, 0);
  return result;
}
...
function getSumOfNumbers(x1, x2, x3) {
  return [x1, x2, x3].reduce((acc, item) => acc + item).toFixed(1);
}
